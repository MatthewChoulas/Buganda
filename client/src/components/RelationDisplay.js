...
export default function DropDown({index, name, relationType, highlight, deleter}) {
    ...
    return (
        <div onMouseEnter={()=> setDeleteActive(true)} onMouseLeave={()=> setDeleteActive(false)} className="flex justify-start cursor-pointer text-gray-700 hover:text-blue-400 hover:bg-blue-100 rounded-md px-2 py-1 my-1">
            ...
            {!deleteActive && <span className={`${highlightColor} h-2 w-2 m-2 rounded-full`}></span>}
            <div className="flex-grow font-sm px-4">{name}</div>
            <div className={`text-sm ${deleteActive ? "text-blue-400" : "text-gray-400"} font-normal tracking-wide`}>{relationType}</div>
        </div>
    )
}